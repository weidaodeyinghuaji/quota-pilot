...
- 设置页支持深色 / 浅色主题切换，悬浮胶囊、详情和更新窗口同步生效并记忆选择。
- 悬浮面板支持极简 / 标准密度切换，标准模式提供刷新、主题和设置快捷操作。
- 官方 Codex 额度低于 20% 或距离 5 小时额度恢复不足 10 分钟时，会发送一次 Windows 提醒。
- 提醒页可调整低额度阈值、恢复前提醒时间、7 天额度提醒与静默时段，并支持发送测试提醒。
- 详情页明确标注额度来自实时官方接口、本地缓存或本地会话估算，并显示更新时间。
- 平台同步采用单任务锁；悬浮面板会显示刷新状态、上次更新时间和低额度文字提示。
